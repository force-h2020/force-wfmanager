...
class WorkflowView(HasTraits):
    ...
    def _error_message_default(self):
        if self.process_view.error_message == '':
            return ERROR_TEMPLATE.format(
                "No errors for current workflow", "")
        else:
            error_list = self.process_view.error_message.split('\n')
    ...
    def map_verify_workflow(self, errors, start_view=None):
        ...
        start_view: Either(HasTraits, ModelView)
        """
        # A dictionary with the mappings between view lists

        mappings = {
            'WorkflowView' : ['process_view', 'mco_view',
                                   'communication_view'],
            'ProcessView': ['execution_layer_views'],
            'ExecutionLayerView': ['data_source_views'],
            'MCOView': ['parameter_views', 'kpi_views'],
            'CommunicationView' : ['notification_listener_views']
        }

        # Begin from top-level WorkflowView if nothing specified already
        #: FIXME: This will cause a infinite loop if any View is not yet
        #: FIXME: initialised
        ...
        if current_view_type in mappings:
            for child_view_list_name in mappings[current_view_type]:
                #print(child_view_list_name)
                child_view_list = getattr(
                    start_view, child_view_list_name
                )
                #print(child_view_list)
                try:
                    for child_view in child_view_list:
                        child_view_errors = self.map_verify_workflow(
                    ...
                    child_view_errors = self.map_verify_workflow(
                        errors, start_view=child_view_list
                    )
