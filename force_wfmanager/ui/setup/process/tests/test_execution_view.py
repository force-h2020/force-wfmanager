...
class TestExecutionLayerModelView(TestProcess):
    ...
    def test_add_data_source(self):
        model = self.factory_1.create_model()
    ...
    def test_remove_data_source(self):
        self.execution_layer_model_view.add_data_source(self.data_sources[0])
        self.assertEqual(
            len(self.execution_layer.data_sources),
            3)

        self.execution_layer_model_view.remove_data_source(self.data_sources[0])
        self.assertEqual(
            len(self.execution_layer.data_sources),
            2)
        self.assertEqual(
